Public Class Form1
    ...
    Private GraphList As New List(Of Graph)
    Private Graph As GraphMovingAverageLine
    Private Graph2 As GraphMovingAverageLine

    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        Me.GraphList = New List(Of Graph)
        Me.Graph = New GraphMovingAverageLine(5)
        Me.Graph2 = New GraphMovingAverageLine(20)
        Me.GraphList.Add(Me.Graph)
        Me.GraphList.Add(Me.Graph2)
        kabu = New StockManager()
        Dim List As KabuList = New KabuList(Me.GraphList)
    ...
    Public Sub DrawKabuList(g As Graphics, ByVal Dates As List(Of Date))
        ...
        Next
        For i As Integer = 0 To Count - 1
            Dim Index As Integer = i + Me.KabuList.Count - Count
            Dim item As KabuItem = KabuList(Index)
            Dim width As Double = PictureBox1.Width / Count
            Dates.Add(item.Time)
            ...
            If Index > 0 Then
                Dim beforeItem As KabuItem = KabuList(Index - 1)
                Dim X2 As Double = width * (i - 1)
                If Me.Graph.Item(item.Time) IsNot Nothing AndAlso Me.Graph.Item(beforeItem.Time) IsNot Nothing Then
                    Dim Point1 As New PointF(x, (1 - ((Me.Graph.Item(item.Time) - Min)) / (Max - Min)) * PictureBox1.Height)
                    Dim Point2 As New PointF(X2, (1 - ((Me.Graph.Item(beforeItem.Time) - Min)) / (Max - Min)) * PictureBox1.Height)
                    g.DrawLine(Pens.Black, Point1, Point2)
                End If

                If Me.Graph2.Item(item.Time) IsNot Nothing AndAlso Me.Graph2.Item(beforeItem.Time) IsNot Nothing Then
                    Dim Point1 As New PointF(x, (1 - ((Me.Graph2.Item(item.Time) - Min)) / (Max - Min)) * PictureBox1.Height)
                    Dim Point2 As New PointF(X2, (1 - ((Me.Graph2.Item(beforeItem.Time) - Min)) / (Max - Min)) * PictureBox1.Height)
                    g.DrawLine(Pens.Black, Point1, Point2)
                End If
